describe("Participant Login and Beginner Level Testing", () => {
  beforeEach(() => {
    cy.viewport("macbook-15");
    cy.visit("http://localhost:3000/login");
  });

  it("Should verify Beginner game elements load properly", () => {
    cy.get('[data-cy="username-input"]').type("Beginner Player");
    cy.get('[data-cy="login-button"]').click();
    cy.get('[data-cy="proficiency-title"]', { timeout: 8000 }).should("be.visible");
    cy.get('[data-cy="beginner-button"]').click();
    ...
    function handleQuestion() {
      cy.get("body").then(($body) => {
        ...
        cy.log(`Handling question #${answeredCount + 1}`);
        ...
        if (answeredCount < 4) {
          ...
          if ($body.find('[data-cy="save-continue-button"]').length > 0) {
            ...
            cy.log("review button not found");
          }
