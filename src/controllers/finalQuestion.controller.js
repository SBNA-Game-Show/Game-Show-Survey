...
const updateFinalQuestions = asyncHandler(async (req, res) => {
  ...
  const insertedQuestions = await questionService.addQuestions(questions, SCHEMA_MODELS.FINALQUESTION);
  return res
    .status(201)
    .json(
      new ApiResponse(
...
const getFinalQuestions = asyncHandler(async (req, res) => {
  ...
  if (req.isAdminRoute) {
    ...
    const questions = await questionService.getQuestionForAdmin(SCHEMA_MODELS.FINALQUESTION);
...
const updateFinalQuestionById = asyncHandler(async (req, res) => {
  ...
  const { questions } = req.body;
  const updatedQuestion = await questionService.updateQuestionById(questions, SCHEMA_MODELS.FINALQUESTION);
  return res
    .status(200)
    .json(
...
const deleteFinalQuestionById = asyncHandler(async (req, res) => {
  ...
  const { questions } = req.body;
  const deletedQuestions = await questionService.deleteQuestionById(questions, SCHEMA_MODELS.FINALQUESTION);

  return res.status(200).json(new ApiResponse(200, deletedQuestions));
});

export { updateFinalQuestions, getFinalQuestions, updateFinalQuestionById, deleteFinalQuestionById }